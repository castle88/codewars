...
function List() {
  this.countSpecDigits = function (integersList, digitsList) {
    if (digitsList.length > 0) {
      const singleDigits = integersList
        .join("")
        .split("")
        .filter((x) => Number(x) >= 0);
      console.log(singleDigits);
      const count = singleDigits.reduce((obj, el) => {
        obj[el] ? obj[el]++ : (obj[el] = 1);
        return obj;
      }, {});

      return digitsList.map((x) => [x, count[x] ? count[x] : 0]);
    }
...
// integersList = [-77, -65, 56, -79, 6666, 222];
// digitsList = [1, 8, 4];
// //console.log(l.countSpecDigits(integersList, digitsList)); //  [[1, 0], [8, 0], [4, 0]]);
